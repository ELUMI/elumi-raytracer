...
void BaseTracer::first_bounce() {
  ...
  for (int y = 0; y < height; y++) {
    for (int x = 0; x < width; x++) {
      ...
      assert(
          material == IAccDataStruct::IntersectionData::NOT_FOUND || material < scene->getMaterialVector().size());
...
vec4 BaseTracer::shade(Ray incoming_ray,
  ...
  case 6: {
    IAccDataStruct::IntersectionData idata2 =
        scene->getAccDataStruct()->findClosestIntersection(incoming_ray);
  ...
  case 7: {
    IAccDataStruct::IntersectionData idata2 =
        scene->getAccDataStruct()->findClosestIntersection(incoming_ray);
    return vec4(
        abs(normalize(idata.normal) - normalize(idata2.normal)) * 100.0f, 1);
  }
  case 8: {
    IAccDataStruct::IntersectionData idata2 =
        scene->getAccDataStruct()->findClosestIntersection(incoming_ray);
  ...
  IAccDataStruct::IntersectionData idatal =
      scene->getAccDataStruct()->findClosestIntersection(lightRay);
  assert(idatal.material != IAccDataStruct::IntersectionData::NOT_FOUND);

  if (dot(dir, (idata.interPoint - idatal.interPoint))
      > 0.001 + settings->test) {
    return vec4();
  } else {
